...
However, we must
% какова цена этого преобразования? почему бы не сделать sizeBound очень маленьким и в ус не дуть?
% (какие возможности оптимизации это преобразование убивает?)
represent configurations in such a way, that knowing \EZY{this sentence is awkward} the behavior of the parts
...
SC~Mini limits the size of configurations in the tree and builds a foldable tree in the following way: \EZY{this sentence is too long, split it up}
if while building the tree a configuration $e$ is encountered, 
which is a function call,
%\footnote{
%Configurations starting with a constructor are always decomposed, 
%giving rise to smaller configurations.},
...
%One of the requirements for an optimizer was to produce
%a new task \texttt{(e', p')}, such that for any arguments
%\texttt{s}:
%\begin{lstlisting}[language=haskell]
%sll_run (e, p) s = sll_run (e', p') s
%\end{lstlisting}
%%\marginpar{DK: Different notation?}
%
%To fulfill this requirement, we create the following program transformer:
...
\EZY{This construction is awkward. My suggestion is take the prior
sentence and push it before the code, and then start off here as ``A most interesting thing happens here'', to highlight it} And the most interesting thing happens there:
...
previously stated definition of ``supercompiler''. \EZY{I dropped the cite}
...
% у Сергея Зефирова в ЖЖ (thesz) где-то было про то, как отличается сила оптимизатора,
% использующего несколько оптимизаций по отдельности (пошагово, пусть даже итеративно)
% и оптимизатора, использующего их "одновременно".
% нашел: http://thesz.livejournal.com/744058.html , см.тж. статью "combining analyses, combining optimizations"
% http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.17.8510 от самого Cliff Click может, сослаться?
...
evaluation\cite{Sorensen1994TurchinSupercompiler,Sorensen1998Introduction}. \EZY{My suggestion here is to define the KMP test immediately, and then describe some of its properties. So jivvy the text around here a bit.}
